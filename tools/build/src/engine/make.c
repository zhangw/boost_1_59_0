...
void make0(
    ...
    int anyhow,
    TARGET *rescanning) /* forcibly touch all (real) targets */
{
    TARGETS *c;
    TARGET *ptime = t;
        ...
        if (DEBUG_FATE)
            if (fate < c->target->fate)
                out_printf("fate change %s from %s to %s by dependency %s\n",
                           ...
                           target_fate[(int)c->target->fate], object_str(c->target->name));
#endif
    }
    ...
    */
    ...
    else if (t->binding == T_BIND_PARENTS && timestamp_cmp(&hlast, &p->time) > 0)
    {
        fate = T_FATE_NEEDTMP;
    }
    ...
    else if (t->binding == T_BIND_EXISTS && p && p->binding != T_BIND_UNBOUND && timestamp_cmp(&t->time, &p->time) > 0)
    {
#ifdef OPT_GRAPH_DEBUG_EXT
        oldTimeStamp = 1;
#endif
    ...
    if (DEBUG_FATE && (fate != savedFate))
    {
        if (savedFate == T_FATE_STABLE)
            ...
            out_printf("fate change  %s from %s to %s%s\n", object_str(t->name),
                       target_fate[savedFate], target_fate[fate], oldTimeStamp ? " (by timestamp)" : "");
    }
#endif
    ...
    if (!(t->flags & T_FLAG_NOTFILE) && (fate >= T_FATE_SPOIL))
        flag = "+";
    else if (t->binding == T_BIND_EXISTS && p && timestamp_cmp(&t->time, &p->time) > 0)
        flag = "*";
...
static void dependGraphOutput(TARGET *t, int depth)
{
    ...
    if (t->flags & ~T_FLAG_VISITED)
    {
        ...
        if (t->flags & T_FLAG_TEMP)
            out_printf("TEMPORARY ");
        if (t->flags & T_FLAG_NOCARE)
            out_printf("NOCARE ");
        if (t->flags & T_FLAG_NOTFILE)
            out_printf("NOTFILE ");
        if (t->flags & T_FLAG_TOUCHED)
            out_printf("TOUCHED ");
        if (t->flags & T_FLAG_LEAVES)
            out_printf("LEAVES ");
        if (t->flags & T_FLAG_NOUPDATE)
